...
def _build_demo_consultation_result(child: dict[str, Any], spec: dict[str, Any], now: datetime) -> dict[str, Any]:
    ...
    return {
        ...
        "healthAgentView": _consultation_agent_view("HealthObservationAgent", "健康观察", spec["healthSignal"], spec["healthSignal"], spec["schoolActions"][0], spec["observationPoints"][0]),
        "dietBehaviorAgentView": _consultation_agent_view("DietBehaviorAgent", "饮食行为", spec["mealSignal"], spec["mealSignal"], spec["schoolActions"][1] if len(spec["schoolActions"]) > 1 else spec["schoolActions"][0], spec["observationPoints"][1] if len(spec["observationPoints"]) > 1 else spec["observationPoints"][0]),
        "parentCommunicationAgentView": _consultation_agent_view("ParentCommunicationAgent", "家园沟通", spec["familySignal"], spec["familySignal"], spec["homeActions"][0], spec["observationPoints"][2] if len(spec["observationPoints"]) > 2 else spec["observationPoints"][0]),
        "inSchoolActionAgentView": _consultation_agent_view("InSchoolActionAgent", "园内执行", spec["schoolSignal"], spec["schoolSignal"], spec["schoolActions"][0], spec["observationPoints"][0]),
        ...
        "continuityNotes": [f"演示样例：补齐 {child['name']} 的连续跟进链路。"],
...
def _build_demo_consultation_feed_item(result: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "explainabilitySummary": {
            ...
            "evidenceHighlights": [text for text in [f"儿童：{child_name}" if child_name else "", f"班级：{class_name}" if class_name else "", *key_findings[:2]] if text],
        },
        ...
        items.append(
            {
                ...
                "explainabilitySummary": {
                    "agentParticipants": ["健康观察", "饮食行为", "家园沟通", "协调中枢"],
                    "keyFindings": [spec["triggerReason"], "需要把园内动作和家庭反馈接成一条线。"],
                    "coordinationConclusion": spec["summary"],
                    "evidenceHighlights": [f"儿童：{child['name']}", f"班级：{child['className']}"],
                },
