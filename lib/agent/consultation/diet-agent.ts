...
export function analyzeDietConsultation(input: ConsultationInput): ConsultationFinding {
  ...
  return {
    ...
    signals: takeUnique([
      `近 7 天平均饮水 ${input.summary.meals.hydrationAvg}ml`,
      ...
      lowWaterDays.length > 0 ? `低饮水记录 ${lowWaterDays.length} 条` : undefined,
    ]),
    actions: takeUnique([
    ...
    evidence: takeUnique([
      ...recentMeals.slice(0, 4).map((item) => `${item.date}${item.meal} 饮水 ${item.waterMl}ml`),
