...
当前比赛阶段，demo 重点路径与当前展示优先级如下：

当前阶段总览：
- Teacher 主线：`T2`、`T2.5`、`T4A`、`T4B`、`T4C`、`T5` 已形成基本闭环；`T5A` 已作为过渡阶段完成其作用；当前进入录屏 / 验收阶段。
- 高风险会诊：仍是当前最强 Agent 工作流展示位。
- Admin：`T9` 已完成展示层接入，Admin 决策区已形成第二展示位。
- Parent：`T10` backend 与 `T11` 展示层已具展示能力，但当前仍是家长侧补强线，不是第一主线。
- `T7A`：backend-ready，但未 UI-ready，前端最小接入仍待补齐。
- `S1.1`：待拿到 staging SSH 后手动执行，不写成 fully healthy / fully switched。
...
- `T2.5` 已完成一轮真机 / 浏览器硬化，权限、手势、fallback 与录音链路已有手工 smoke 口径。
...
目标：承接高风险会诊结果，把多智能体会诊结论压缩成园长办公会式的决策卡与优先级面板。

当前可复用落点：
- 园长首页：`app/admin/page.tsx`
- 园长 Agent：`app/admin/agent/page.tsx`
- 决策 / trace 相关展示：`components/admin/*`、`components/consultation/*`

当前口径：
- `T9` 已完成 Admin 展示层接入，当前应写成园长侧第二展示位。
- 不要把它扩写成独立后端聚合、staging fully healthy 或远端链路已 fully switched。
- 它和高风险会诊一起构成当前园长侧答辩叙事，不替代高风险会诊主线本身。

### 家长侧补强展示线：Parent 趋势问答 / TrendLineChart
目标：作为家长侧补强展示线，把 7 / 14 / 30 天时间窗趋势、解释文本与对话追问压缩进移动端聊天流，增强比赛演示时的“时光穿梭机”感知入口。

当前可复用落点：
- 家长首页：`app/parent/page.tsx`
- 家长 Agent：`app/parent/agent/page.tsx`
- 干预卡：`components/agent/InterventionCardPanel.tsx`
- 趋势图卡：`components/parent/TrendLineChart.tsx`、`components/parent/ParentTrendResponseCard.tsx`
- 趋势接线：`lib/agent/parent-trend.ts`、`app/api/ai/parent-trend-query/route.ts`
- 状态与反馈：`lib/store.tsx`

当前口径：
- 这条路径当前应写成“基于现有 parent home + parent agent + T10 backend 的 TrendLineChart + Parent 对话界面增强线”。
- `T10` 已完成 backend（代码层），`T11` 已完成 Parent 展示层，但这不改变当前最优先主线，仍不能写成 live 趋势平台或 staging / vivo 已 fully live。
- “时光穿梭机 / 微绘本”继续保留为后续情感化增强方向，不要写成当前最优先主攻线。
...
| `T2.5` | 已完成 | Teacher 语音入口已完成一轮真机 / 浏览器硬化与 smoke 口径沉淀 |
| `T3` | 已完成 | MySQL / SQLite / memory fallback 方向的记忆层基础已完成 |
| `T4` | 已拆分 | `T4A` 完成；`T4B` 完成但 live upstream 未 fully verified；`T4C` 已完成 |
| `T5` | 已拆分 | `T5A` 已作为过渡阶段完成其作用；`T5` 已完成主接线，但仍未写成 fully verified |
| `T6` | 已完成 | Tool layer + ReAct runner + trace 已具备可演示能力 |
| `T7` | 部分具备基础 | `T7A` 已完成 backend-first 版本，当前为 backend-ready；不是 UI-ready；前端最小接入仍未完成 |
| `T8` | 部分具备基础 | 高风险会诊已有强金链路，但不要写成完整 `T8` 已完成 |
| `T9` | 已完成（Admin 展示层） | Admin 决策区 / 风险优先级 / 会诊 trace 面板已接入 `/admin` 与 `/admin/agent`，作为第二强展示位 |
| `T10` | 已完成（代码层） | Parent 趋势问答 backend 已完成，支持 7 / 14 / 30 天聚合与 `request_snapshot` / `remote_snapshot`，但仍受 snapshot / fallback 约束 |
| `T11` | 已完成（Parent 展示层） | TrendLineChart + Parent 对话界面已接入 `/parent/agent`，当前已具展示能力，但仍属于家长侧补强线 |
...
- 当前可保守确认：DNS 已解析到 `api-staging.smartchildcareagent.cn`，并已看到远端 JSON 返回及 vivo / memory 相关链路的局部证据；但域名 / TLS 最终打通、新 health schema 对外可见、真 SSE 验证闭环、release URL remote-brain-proxy 真验收仍未完成。
