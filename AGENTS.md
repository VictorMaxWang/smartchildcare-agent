...
### 主线 3：Parent 趋势问答 / TrendLineChart 展示增强线
...
- `T10` 已完成 backend，`T11` 已完成 Parent 展示层，但这不改变当前最优先主线，仍不能写成 staging / vivo 已 fully live。
...
| `T10` | 已完成（代码层） | Parent 趋势问答 backend 已完成，支持 `request_snapshot` / `remote_snapshot` 与 `demo_snapshot` fallback |
| `T11` | 已完成（Parent 展示层） | TrendLineChart + Parent 对话界面已接入 `/parent/agent`，用于移动端趋势问答展示，但不改变当前最优先主线 |
