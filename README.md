...
- 普通账号注册/登录和 `/api/state` 现在统一使用 `mysql2/promise` 连接 MySQL。
- 示例账号登录继续不依赖数据库。
- `DATABASE_URL` 请使用 MySQL 连接串，例如 `mysql://childcare_app:password@db.example.com:3306/childcare`。
- 云数据库要求 TLS 时设置 `DATABASE_SSL=true`，本地无 TLS 可留空或设为 `false`。
- 所有部署环境都必须配置 `AUTH_SESSION_SECRET`，自定义 Cookie 会话依赖它。
- SQL 初始化脚本仍放在 `supabase/sql/` 目录中，但内容已经改为 MySQL DDL。
