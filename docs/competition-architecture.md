...
- 当前代码事实：
  ...
  - Storybook 页面本体、T12 一键播报 / 语音反馈，以及账户级祖辈偏好仍是后续阶段，不在本轮宣称已完成。
...
- 当前事实：
  ...
  - `T23` 再把 Teacher / Parent / Storybook / Intervention 主链全面接到同一策略层，不在 `T22` 里提前扩散 UI 改造。
...
- 当前代码事实：
  ...
  - 当前前端只承接轻量 preview，不把 `/admin/agent?action=weekly-report` 重做成第二套首页，也不宣称 fully live；后续 walkthrough / richer drill-down 仍属于 `T27` 之后的细化工作
...
- Deferred to T23：
  - Teacher / Parent / Storybook / Intervention 主链的页面级接入与 walkthrough
