...
- 当前代码事实：
  ...
  - `T19` 继续负责 Teacher / Admin evidence chain 的 UI 接入，不与本轮 contract / mapping 混写
  - `lib/consultation/evidence.ts`
  - `lib/consultation/trace-types.ts`
  - `lib/consultation/trace-view-model.ts`
...
- 当前代码事实：
  - `T18` 已把 consultation result、admin feed、trace view model 统一挂到 `evidenceItems`
  - `T19` 才负责把结构化 evidence 变成 Teacher / Admin 的证据链 UI，不应把本轮写成 UI 已完成
...
- 当前消费落点：
  ...
  - 旧 `evidenceHighlights` 与 stage legacy evidence 仍保留为兼容投影
